...
#define IS_VALID_COLLISIONCHANNEL(x) ((x)> ECC_NavigationRelevant && (x) < ECC_OverlapAll_Deprecated)
...
void UCollisionProfile::LoadProfileConfig(bool bForceInit)
{
	...
	for ( int32 EnumIndex=0; EnumIndex<NumEnum; ++EnumIndex )
	{
		...
		else
		{
			...
			if (CollisionChannel == ECC_Visibility || CollisionChannel == ECC_Camera || CollisionChannel == ECC_NavigationRelevant)
			{
